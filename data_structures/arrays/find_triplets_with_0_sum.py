...
def find_triplets_with_0_sum_hashing(nums: list[int]) -> list[list[int]]:
    ...
    Return:
          list of list of integers where sum(each_list) == 0
    ...
    for index, item in enumerate(nums[:-2]):
        ...
        # Traverse the subarray nums[i+1:].
        for other_item in nums[index + 1:]:
            # required value for second element
            required_value = current_sum - other_item

            # verify if the desired value exist in the set.
            if required_value == set_initialize:
